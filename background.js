...
class ImageDownloader {
  ...
  async startDownload(message) {
    ...
    try {
      ...
      this.sendProgress(
        10,
        `Found ${pages.length} pages to crawl`,
      ...
      this.sendProgress(
        50,
        `Found ${allImages.size} unique images`,
        `Found ${allImages.size} images`,
        null,
        null,
        pages.length,
        allImages.size,
  ...
  async crawlWebsite(startUrl) {
    ...
    try {
      ...
      while (pages.size < maxPages) {
        const pagesToProcess = Array.from(pages).filter(
          (url) => !visited.has(url),
        );
      ...
      elements.forEach((img) => {
        ...
        if (src && !src.startsWith("data:")) {
          try {
            ...
            const filename =
              this.getFilenameFromUrl(imageUrl.href) ||
              `image_${Date.now()}.jpg`;
          ...
          if (srcset) {
            ...
            urls.forEach((imgUrl) => {
              try {
                ...
                const filename =
                  this.getFilenameFromUrl(fullUrl.href) ||
                  `image_${Date.now()}.jpg`;
                ...
                if (!images.find((img) => img.url === fullUrl.href)) {
                  images.push({
                    ...
                    title: img
                      ? img.title || img.getAttribute("title") || ""
                      : "",
                    type: "picture-source",
                    sourcePage: url,
                  });
